    ...
    static class NuGetPluginConfigurator
    {
        ...
        public static void ConfigureDll(string assetPath, bool includeInBuild)
        {
            ...
            else
            {
                ...
                // Unity preserves the Any Platform "Exclude Editor" bit even after
                // Any Platform itself is disabled. If we leave the stale bit set,
                // currentExcludeEditor never matches the expected false value above,
                // so every domain reload calls SaveAndReimport again and creates an
                // endless reload loop.
        ...
        /// Transitive dependencies default to editor-only so a package update cannot
        /// silently put an MCP dependency into a player build.
        /// </summary>
        static bool ShouldIncludeInBuild(string packageId)
        {
            ...
            // Transitive dependency — default to editor-only.
            return false;
        }
    }
}
